...
scoreboard.score();
scoreboard.help('Mozogj a nyilakkal.' +
...
function kereskincset() {
    if (fa_kinccsel == undefined)
        return;
    var kincses_fa = fak[fa_kinccsel], p1 = kincses_fa.parent.position, p2 = marker.position;
    var distance = Math.sqrt((p1.x - p2.x) * (p1.x - p2.x) +
        (p1.z - p2.z) * (p1.z - p2.z));
    if (distance < 500) {
        scorePoints();
    }
...
function makeTreeAt(x, z) {
    ...
    top.position.y = 175;
    trunk.add(top);
    var boundary = new THREE.Mesh(new THREE.CircleGeometry(270), new THREE.MeshNormalMaterial());
...
function animateFruit() {
    if (fruit)
        return;
    fruit = new THREE.Mesh(new THREE.CylinderGeometry(25, 25, 5, 25), new THREE.MeshBasicMaterial({ color: 0xFFD700 }));
